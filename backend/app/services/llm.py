...
def _build_trip_plan_prompt(preferences: Dict[str, Any]) -> tuple[str, str]:
    ...
    parts = [
        ...
        f"  FLIGHT: international.min/max are the TOTAL round-trip flight cost for ALL {group_size} traveler(s) combined. ",
        "international.min = cheapest connecting/budget-carrier round-trip fare for the whole group. ",
        "international.max = direct peak-season economy fare for the whole group. ",
        "Connecting flights are typically 25-45% cheaper than direct; note this in international.note. ",
        f"Group flight cost (all {group_size} people): CA/US<->Asia {group_size*950}-{group_size*2500}; CA/US<->Europe {group_size*700}-{group_size*1800}; ",
        f"CA/US<->Africa/ME {group_size*1100}-{group_size*2800}; CA/US<->S.America {group_size*600}-{group_size*1600}; intra-continent {group_size*150}-{group_size*600}. ",
        f"Example: for {group_size} traveler(s) at 800/person, international.min = {group_size*800}, international.max = {group_size*1100}.\n",
        "  OWN_CAR: international.min/max = total round-trip fuel cost for ALL travelers (not per person). ",
        ...
        f"  CAR_RENTAL: international.min = economy rental ~45-65 CAD/day x {nights} nights + low fuel (total for all). ",
        f"international.max = mid-size rental ~80-120 CAD/day x {nights} nights + high fuel (total for all). ",
        "international.note = 'Approx X CAD/day rental + fuel. Book in advance for best rates.'\n",
        "  BUS_TRAIN: international.min/max = total round-trip ticket cost for ALL travelers. ",
        "international.note = 'Round-trip ticket estimate for the group.'\n",
        ...
        "IMPORTANT: transport_total is the FULL cost for ALL travelers combined. ",
        "Do NOT set transport_total to only local transport.\n",
        "- transportation_options[0].priceEstimate.min/max = international.min/max divided by group_size (per-person price).\n",
        "- grand_total = hotels_total + activities_total + food_total + transport_total + shopping_misc_total. ",
        "transport_total includes ALL international travel + local transport for ALL travelers.\n",
